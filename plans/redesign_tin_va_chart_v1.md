...
| 3 | ~~OI re-enable~~ **DONE 2026-07-10, better than planned**: `probe_oi_slope()` (hang-proof via `_bounded_get` daemon-thread hard-deadline, which already existed) consulted AT TRIGGER TIME only on flush hits, ≤3/cycle → splits **`flush_oi_dn`** (the +EV lab setup, now measured live from R1 dark mode) from `flush_no_oi`. Measured ~1.6s/probe live; worst ≤20s/probe (2 endpoints × 10s hard cap) only during API failure + flush cluster | S | flush is rare (ret5≤−3% + vol≥2×) → ~zero cost on normal cycles; cost lands exactly when the info matters |
