...
def paper_runtime_status(heartbeats: list[dict]) -> dict:
    ...
    tracked = [
        ...
        lookup.get("learning_exam_benchmark"),
        lookup.get("test_result_memory_agent"),
...
def compact_shadow_performance(performance: dict) -> dict:
    ...
    fresh_window = performance.get("fresh_window") if isinstance(performance.get("fresh_window"), dict) else {}
    fresh_overall = fresh_window.get("overall") if isinstance(fresh_window.get("overall"), dict) else {}
    fresh_quality = fresh_window.get("data_quality") if isinstance(fresh_window.get("data_quality"), dict) else {}
    ...
    return {
        ...
        "fresh_window": {
            "start_ts": fresh_window.get("start_ts"),
            "row_count": int(fresh_window.get("row_count", 0) or 0),
            "assumption_hash": fresh_window.get("assumption_hash") or "none",
            "closed": int(fresh_overall.get("closed", 0) or 0),
            "wins": int(fresh_overall.get("wins", 0) or 0),
            "losses": int(fresh_overall.get("losses", 0) or 0),
            ...
            "ambiguous_count": int(fresh_quality.get("ambiguous_count", fresh_overall.get("ambiguous_count", 0)) or 0),
        },
...
def compact_experiments(payload: dict) -> dict:
    ...
    return {
        ...
        "experiment_count": int(payload.get("experiment_count") or len(rows)),
        "by_status": by_status,
...
def compact_ops() -> dict:
    return {
        ...
        "experiments": compact_experiments(read_json(MEMORY_DIR / "walk_forward_latest.json") or read_json(MEMORY_DIR / "experiments_latest.json")),
