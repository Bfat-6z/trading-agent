"""Render BSB, JUP, HYPE charts 15m+1h to study visually."""
...
def render(sym, save_path):
    k_15m = fetch_klines(sym, "15m", 80)
    k_1h = fetch_klines(sym, "1h", 60)

    fig, axes = plt.subplots(2, 2, figsize=(16, 10))

    for col, (k, label) in enumerate([(k_15m, "15m"), (k_1h, "1h")]):
        ax_p = axes[0, col]
        ax_v = axes[1, col]

        closes = [b["c"] for b in k]
        times = [b["t"] for b in k]
        ema20 = ema(closes, 20)
        ema50 = ema(closes, 50)
        rsis = rsi(closes, 14)
        ...
        for i, b in enumerate(k):
            ...
            ax_p.bar(times[i], b["c"]-b["o"], width=0.008 if label=="15m" else 0.03, bottom=b["o"], color=color, alpha=0.7)

        ax_p.plot(times, ema20, color="blue", linewidth=1, label="EMA20")
        ax_p.plot(times, ema50, color="orange", linewidth=1, label="EMA50")
        ax_p.set_title(f"{sym} {label} | Close: ${closes[-1]:.4f} | RSI14: {rsis[-1]:.1f} | EMA20: ${ema20[-1]:.4f}")
        ...
        ax_v.bar(times, vols, color=colors, alpha=0.6, width=0.008 if label=="15m" else 0.03)
        ax_v.set_ylabel("Volume", color="gray")
        ax_v.grid(alpha=0.3)
        ax_v.xaxis.set_major_formatter(DateFormatter("%m-%d %H:%M"))
        plt.setp(ax_v.xaxis.get_majorticklabels(), rotation=45, fontsize=8)

        ax_rsi = ax_v.twinx()
        ax_rsi.plot(times, rsis, color="purple", linewidth=1.2, label="RSI14")
...
for sym in ["BSBUSDT", "JUPUSDT", "HYPEUSDT"]:
    try:
        render(sym, f"charts/{sym}_multi_tf.png")
