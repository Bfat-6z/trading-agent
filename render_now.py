"""Render multi-TF charts for HYPE, EDEN, GRASS, BTC for visual analysis."""
...
def render(sym, save_path):
    ...
    for col, (interval, label, w) in enumerate([("15m", "15m", 0.008), ("1h", "1h", 0.03)]):
        ...
        ax_p.set_title(f"{sym} {label} | Close: ${closes[-1]:.5f} | RSI14: {rsis[-1]:.1f} | EMA20: ${ema20[-1]:.5f}", fontsize=11)
        ...
        ax_v.set_ylabel("Volume", color="gray"); ax_v.grid(alpha=0.3)
...
for sym in ["HYPEUSDT", "EDENUSDT", "GRASSUSDT", "BTCUSDT"]:
    try:
        render(sym, f"charts/{sym}_now.png")
