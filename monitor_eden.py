"""Monitor EDENUSDT SHORT — $1 target challenge."""
...
last_b = None      # +$0.5 / +$1.0 milestones
last_dd = None     # -$0.30 alerts
...
print(f"MONITOR EDEN_SHORT entry=${ENTRY} target=${TP_TARGET} sl=${SL_TRIGGER}", flush=True)
while True:
    try:
        ...
        if last_b is None or new_b != last_b:
            if new_b > 0 and (last_b is None or new_b > last_b):
                ...
                print(f"PROFIT_GAVE_BACK +${pnl:.4f} mark=${mark}", flush=True)
            last_b = new_b
        new_dd = dd(pnl)
